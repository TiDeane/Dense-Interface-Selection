...
function drawUserIDScreen() {
  ...
  explanation = createDiv(
    "Os targets estão divididos em 4 grupos: frutas, sumos, vegetais e leites."
  );
  explanation.id("explanation");
  explanation.position(100, 170);

  explanation_2 = createDiv(
    "Além disso, eles estão organizados por ordem alfabética dentro de cada grupo."
  );
  explanation_2.id("explanation");
  explanation_2.position(100, 188);

  explanation_3 = createDiv(
    "Lembrem-se que o tempo só começa a contar após clicarem no primeiro target!"
  );
  explanation_3.id("explanation");
  explanation_3.position(100, 206);
  ...
  start_button.position(
    ...
    height / 2 - start_button.size().height / 2 + 5
  );
}
...
function startTest() {
  if (validID() && validSize()) {
    ...
    start_button.remove();
    explanation.remove();
    explanation_2.remove();
    explanation_3.remove();
